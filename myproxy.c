/*
 ...
 UCID: 10083186
 CPSC 441 - Assignment 1 - Feb 2nd, 2018
...
accepting:
    ...
    if(pid==0)
    {
        ...
        char buffer[510],t1[300],t2[300],t3[10], setting[10];
        char* temp=NULL;
        memset(buffer, '\0', 500);
        ...
        if(((strncmp(t1,"GET",3)==0))&&((strncmp(t3,"HTTP/1.1",8)==0)||(strncmp(t3,"HTTP/1.0",8)==0))&&(strncmp(t2,"http://",7)==0))
        {
            ...
            if(strncmp(setting,"normal",6)==0)
            {
                ...
                if(temp!=NULL){
                    ...
                    sprintf(buffer,"301: PAGE MOVED PERMANENTLY/n",t3,t2);}
                
                
                n=send(sockfd1,buffer,strlen(buffer),0);
                printf("\n%s\n",buffer);
                if(n<0)
                    error("206: PARTIAL CONTENT,CANNOT DISPLAY./n");
                else{
                    do
                    {
                        printf("200: OK./n");
